...
async def _get_class_or_404(class_id: str) -> dict:
    cls = await classes_collection.find_one({"_id": _oid(class_id, "class id")})
    if not cls:
...
async def _assert_can_view(cls: dict, current_user: dict):
    ...
    if role == "student":
        enrollment = await enrollments_collection.find_one(
            {"course_id": cls["course_id"], "student_id": current_user["user_id"]}
        )
        if enrollment:
            return
...
async def create_class(payload: ClassCreateRequest, current_user: dict = Depends(get_current_user)):
    ...
    enrollments = await enrollments_collection.find({"course_id": payload.course_id}).to_list(length=1000)
...
async def list_classes(current_user: dict = Depends(get_current_user)):
    ...
    if role == "admin":
        ...
        enrollments = await enrollments_collection.find({"student_id": current_user["user_id"]}).to_list(length=1000)
...
async def start_class(class_id: str, current_user: dict = Depends(get_current_user)):
    ...
    enrollments = await enrollments_collection.find({"course_id": cls["course_id"]}).to_list(length=1000)
    for e in enrollments:
        await create_notification(
            user_id=e["student_id"],
            type="class_live",
