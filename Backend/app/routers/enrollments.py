...
async def enroll_student(course_id: str, payload: EnrollRequest, current_user: dict = Depends(require_role("admin"))):
    ...
    student = await users_collection.find_one({"_id": _oid(payload.student_id, "student id"), "role": "student"})
    if not student:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Student not found")

    existing = await enrollments_collection.find_one({"course_id": course_id, "student_id": payload.student_id})
    if existing:
...
async def unenroll_student(course_id: str, student_id: str, current_user: dict = Depends(require_role("admin"))):
    result = await enrollments_collection.delete_one({"course_id": course_id, "student_id": student_id})
...
async def list_course_enrollments(course_id: str, current_user: dict = Depends(get_current_user)):
    ...
    enrollments = await enrollments_collection.find({"course_id": course_id}).to_list(length=1000)

    out = []
    for e in enrollments:
        student = await users_collection.find_one({"_id": ObjectId(e["student_id"])})
        if student:
...
async def list_my_students(current_user: dict = Depends(require_role("teacher"))):
    ...
    courses = await courses_collection.find({"teacher_ids": current_user["user_id"]}).to_list(length=1000)
    course_ids = [str(c["_id"]) for c in courses]
    title_by_course_id = {str(c["_id"]): c["title"] for c in courses}

    enrollments = await enrollments_collection.find({"course_id": {"$in": course_ids}}).to_list(length=5000)
